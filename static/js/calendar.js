const addVaccine = async (id) => {
    let curr_user = await getJSON("/current_user/");
    let date = new Date();
...
const getPastDate = (months, birth_date) => {
...
const differenceDate = (date) => {
    let today = new Date();
    let year_dif = Math.abs(date.getFullYear() - today.getFullYear());
    if (year_dif) {
        if (year_dif === 1) {
            return "Через " + year_dif.toString() + " рік"
        }
        return "Через " + year_dif.toString() + " років";
    }
    let month_dif = Math.abs(date.getMonth() - today.getMonth());
    if (month_dif) {
        if (month_dif === 1) {
            return "Через " + month_dif.toString() + " місяць"
        }
...
const postDate = (data, id) => {
    ...
    el.innerHTML = `
                    <section class="Summary-block__content-cell">
                        <h5 class="Summary-block__sub-title">${vaccines[data["vacc_id"]-1]}</h5>
                                                <button class="Summary-block__button"
                            onclick="addVaccine(${data["vacc_id"]});window.location.reload()">
                            <span class="Summary-block__sub-text">Зробити</span>
                            <svg class="Summary-block__icon" viewBox="0 0 32 32" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path d="M22.672 3.82807C22.2447 4.20196 22.2447 4.80828 22.672 5.1824L28.0901 9.92324C28.5174 10.2971 29.2103 10.2971 29.6379 9.92324L31.1859 8.56845L24.22 2.47328L22.672 3.82807Z"
                                  fill="#05C46B"/>
                            <path d="M21.1241 9.2456L24.22 6.53671L26.542 8.56843L23.4461 11.2773L21.1241 9.2456Z"
                                  fill="#D1D3D4"/>
                            <path d="M20.3503 17.3727L12.6105 10.6004L4.87016 17.3727C4.01529 18.1207 4.01529 19.3336 4.87016 20.0816L6.03154 21.0973L7.96629 22.7907L11.0622 25.4996C11.9173 26.2478 13.3034 26.2478 14.1583 25.4996L17.2542 22.7907L21.8981 18.7268L20.3503 17.3727Z"
                                  fill="#66BFFF"/>
                            <path d="M21.1242 9.24559L17.6411 6.19789L12.6105 10.6004L20.3503 17.3727L21.8981 18.7268L26.9292 14.325L21.1242 9.24559Z"
                                  fill="#F1F2F2"/>
                            <path d="M13.2498 25.4991C13.385 25.6172 13.5379 25.7187 13.7041 25.8008C12.8468 26.2366 11.7616 26.1127 11.0607 25.4991L7.96854 22.7886L6.03112 21.0983L4.8708 20.083C4.01727 19.3341 4.01727 18.1217 4.8708 17.3727L12.6095 10.6013L13.7041 11.5591L7.05996 17.3727C6.20642 18.1217 6.20642 19.3341 7.05996 20.083L8.22027 21.0983L10.1577 22.7886L13.2498 25.4991Z"
                                  fill="#4E7FFD"/>
                            <path d="M12.61 10.6011L17.6395 6.2L18.7341 7.15775L13.7043 11.5588L12.61 10.6011Z"
                                  fill="#E6E7E8"/>
                            <path d="M4.48326 22.4521C4.05596 22.826 4.05596 23.4323 4.48326 23.8062L5.64464 24.822L6.41854 25.4996L6.80549 25.8382C7.23306 26.2121 7.92572 26.2121 8.35329 25.8382L9.90163 24.4834L7.96634 22.7907L6.03159 21.0974L4.48326 22.4521Z"
                                  fill="#05C46B"/>
                            <path d="M4.87021 24.1448L1.82913 26.8065C0.823805 27.6859 0.369246 28.9376 0.609753 30.1635L0.680302 30.521L6.41855 25.4996L5.64465 24.822L4.87021 24.1448Z"
                                  fill="#D1D3D4"/>
                        ...
                        <p class="Summary-block__sub-text">${differenceDate(data["date"])}</p>
                    </section>`;
    return el;
};
...
const initCalendar = async () => {
    ...
    vaccines.map(x => {x["date"] = new Date(ConvertToSlash(x["date"]))});
    // vaccines.map((x) => {x["age"] = getPastDate(x["age"], ConvertToDMY(client_birth))});
    // vaccines = vaccines.filter(x => x["age"]);

    vaccines.sort((x, y) => {return compareDates(x["date"], y["date"]);});
    // vaccines.map(x => {x["vacc_id"] = vaccines[x["vacc_id"] - 1]});
